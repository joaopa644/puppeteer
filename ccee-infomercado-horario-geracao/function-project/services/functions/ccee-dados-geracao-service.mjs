...
import CCEEDadosGeracaoPage from '../../pages/ccee/dados-analises/dados-geracao-page.mjs';
...
export default class CCEEDadosGeracaoService{
    #browserService;
    #context;
    #fileDownload;
    #blobStorage;
    ...
    async startProcess(currentMonthYear){
        try{
            ...
            const dadosGeracaoPage = new CCEEDadosGeracaoPage(page, this.#context);

            await dadosGeracaoPage.open();

            const downloadUrl = await dadosGeracaoPage.getUrlDownloadDadosAnalise(currentMonthYear);
    ...
    async #uploadFile(buffer, fileName){
        const containerName = process.env['CCEE_HORARIO_GERACAO_CONTAINER'];
    ...
    getMonthYearCurrentDadosAnalise(){
        const date = new Date();
